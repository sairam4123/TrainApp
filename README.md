...
`Sim.Run()` itself is now a thin loop: it pulls the next event, logs it, and hands it to the event's train's `TrainController.OnEvent`, which contains the actual state-machine logic (this used to all live inline in `Sim.Run()`). This separation allows the system to reason about time explicitly and to update state only when meaningful events occur.

Movement authority (`MovementAuthorized` / `MovementAuthorityEnded`) is defined as an event type in [events.go](railway/events.go), and a `MovementAuthority` type exists in [authority.go](railway/authority.go) to hold and extend a train's authorized path. `Train` holds its currently-granted authority (`Train.ma`), set whenever `Dispatcher.RequestToProceed` succeeds, but both event cases in `TrainController.OnEvent` are still empty stubs, and `MovementAuthority.ExtendByPath`/`NewMovementAuthority` are never called — authorities are granted wholesale per reservation, never extended or explicitly ended. `ScheduleEnd`, by contrast, is now a real, handled event: it fires once a train has no more schedule points left and releases its last track/reservation before `WorldExited`.
...
- **Commented out, kept for reference**: an earlier `func main` that calls `worlds.BuildTestWorld()`, the older 3-station (TPJ-PDKT-KKDI) world, and runs a larger set of trains through the same `sim.Init()` / `sim.Run()` flow.
...
- **The proceed (switch-lock) retry queue is never drained.** When `Dispatcher.RequestToProceed` fails — either `Path.EnsureAllEdgesAreReserved` or `Interlocking.EnsureAllSwitchesLocked` returns false — the request is appended to `Dispatcher.waitingProceedRequests`, but nothing ever reads from that slice again. Unlike `waitingReservationRequests`, which `OnTrackReleased` retries on every track release, a train stuck here just stays stuck; it doesn't even get the FIFO-unfairness treatment above. In the sample world this is a real, not just theoretical, dead end: running `go run .` currently ends with several trains permanently stuck retrying a reservation that never frees up (see the `Failed to unlock...`/`waiting...` lines at the end of a run).
- **Movement authority is tracked but inert.** `Train.ma` holds the currently-granted `MovementAuthority` (set whenever `RequestToProceed` succeeds), but both the `MovementAuthorized` and `MovementAuthorityEnded` cases in `TrainController.OnEvent` are empty stubs, and `MovementAuthority.ExtendByPath`/`NewMovementAuthority` are never called — authorities are never extended or ended, they're just replaced wholesale by the next full-path reservation.

Two issues from an earlier pass have since been fixed and are no longer limitations: `Interlocking.trackSwitchMap` is now `map[string][]string` (every switch block owning a track ID gets locked/unlocked together, not just the last one registered), `NewTrackID` normalizes point order so both directions across a diamond crossing now share the same track ID as the diamond and get jointly locked, and the standalone `PointController` locking path has been deleted outright (rather than left orphaned) along with the rest of `points.go`'s old contents.
