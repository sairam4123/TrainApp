...
│   ├── testWorld.go     # Older, partly-commented-out 3-station world (TPJ-PDKT-KKDI)
│   └── kkdiTpjWorld.go  # Active double-line TPJ-PDKT-KKDI world with crossovers, signals, and candidate-path generation
├── railway/             # Core simulation code
│   ├── world.go         # Main container for stations, tracks, trains, and state
│   ├── train.go         # Train model, schedule, and TrainController (per-train event handling)
│   ├── stations.go      # Station and platform definitions
│   ├── tracks.go        # Track segments and route pieces
│   ├── points.go        # TrackPoint: topology node (dead-end / sim-boundary flags only)
...
│   ├── authority.go     # MovementAuthority type for granting a train's authorized path
...
│   ├── sim.go           # Discrete-event loop; dispatches each event to the owning train's TrainController
...
### 1. World
The World object is the top-level container. It holds the full railway model, including stations, tracks, points, switch blocks, signals, and trains. It is the shared context in which all other components operate.
...
- `TrackGraph.GenerateCandidatePaths` enumerates every possible path between a starting point and a target track; the interlocking (see below) sorts these candidates and reserves the first one that succeeds. The single-path `FindPath` / `FindPathToTrack` are marked `@Deprecated` and aren't called by the dispatcher
...
### 3. Train model
Each train has a schedule and a lifecycle that can be understood as a sequence of state transitions. `Train` itself is a passive record (identity, schedule, current occupation/reservation/authority); the state machine that drives it lives in `TrainController.OnEvent` ([train.go](railway/train.go)), one instance per train, created in `Sim.Init()` and looked up by train number as events arrive. It can:
...
### 4. Control layer
This layer makes the system safe. `TrainController` requests a route through one of two `Dispatcher` entry points: `RequestRouteToPlatform`, used only on world entry or when a movement authority has ended off-platform, and `RequestRouteToStation`, used for onward travel once a train is already sitting in a station platform (it falls back to the best signal-bounded leg toward the platform if the platform itself can't be reserved). Both ultimately ask the interlocking (`Interlocking.TryReservePathTo`) to find a path: it generates every candidate path to the target track, sorts them by whether they navigate in a signal-correct direction and then by distance, and attempts to reserve tracks and lock switches along the best candidate, falling back to the next one if reservation fails. If no candidate can be reserved, the request is queued and retried when a track is released. In practice, this is the layer that turns abstract topology into coordinated, conflict-free train operation.
...
`Sim.Run()` is a thin loop: it pulls the next event, logs it, and hands it to the event's train's `TrainController.OnEvent`, which contains the actual state-machine logic. This separation allows the system to reason about time explicitly and to update state only when meaningful events occur.

Movement authority (`MovementAuthorized` / `MovementAuthorityEnded`) is defined as an event type in [events.go](railway/events.go), and a `MovementAuthority` type exists in [authority.go](railway/authority.go) to hold a train's authorized path. `Train` holds its currently-granted authority (`Train.ma`), set whenever `Dispatcher.RequestToProceed` succeeds. `MovementAuthorized` acquires the first track of the newly granted path — either because the train is entering the world, or, mid-route, whenever the train's current track isn't a station platform — and otherwise (train sitting at a platform) increments the schedule point and fires `TrainDeparted`; `MovementAuthorityEnded` looks up the train's current schedule point and calls `RequestRouteToStation` to reserve onward travel. Authorities are still granted wholesale per reservation rather than incrementally extended. `ScheduleEnd` fires once a train has no more schedule points left and releases its last track/reservation before `WorldExited`.

## main.go: work in progress

[main.go](main.go) contains two versions of the entry point:

- **Active (`func main`)**: calls `worlds.BuildTpjKkdiWorld()`, which builds a three-station, double-line world — TPJ, an intermediate PDKT, and KKDI, each with four platforms and a crossover — connected by two inter-station legs (TPJ↔PDKT ~55km, PDKT↔KKDI ~41km), with a signal guarding entry at each end of both legs. It creates two trains per iteration (6 iterations, i.e. 12 trains) with schedules via `Train.AddSchedule`, adds them to the world, and runs the full discrete-event simulation with `sim.Init()` / `sim.Run()`.
- **Commented out**: an alternate `func main` that calls `worlds.BuildTestWorld()`, a 3-station (TPJ-PDKT-KKDI) world, and runs a larger set of trains through the same `sim.Init()` / `sim.Run()` flow.
...
1. The railway world is built (see [worlds/](worlds/)) with stations, platforms, tracks, points, and signals.
...
- **No fairness in the proceed (switch-lock) retry queue either.** When `Dispatcher.RequestToProceed` fails — either `Path.EnsureAllEdgesAreReserved` or `Interlocking.EnsureAllSwitchesLocked` returns false — the request is appended to `Dispatcher.waitingMaRequests`, and `OnTrackReleased` does retry it on every track release, but nothing prioritizes a train that keeps losing out, so it can be starved the same way as the reservation queue above. In the sample world this is a real, not just theoretical, dead end: running `go run .` currently ends with several trains permanently stuck retrying a reservation that never frees up (see the `Failed to unlock...`/`waiting...` lines at the end of a run).
- **Movement authority is always granted wholesale, never extended.** `Train.ma` holds the currently-granted `MovementAuthority` (set whenever `RequestToProceed` succeeds); see [Simulation engine](#5-simulation-engine) for how `MovementAuthorized`/`MovementAuthorityEnded` are handled. Each new authority is still a full, freshly-reserved path rather than an extension of the previous one — there's no partial-authority or "extend up to the next signal" concept yet.
